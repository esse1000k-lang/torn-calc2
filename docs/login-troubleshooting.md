...
- 로그인한 **같은 브라우저**에서 `https://사이트주소/api/debug-auth` 를 연다.
- 응답 예: `{ hasCookie: true, hasSignedSession: false, hasUser: false, hint: "..." }`
  - `hasCookie: false` → 브라우저가 쿠키를 안 보냄 (도메인/HTTPS/주소 통일 문제).
  - `hasCookie: true`, `hasSignedSession: false` → 쿠키는 오지만 **서명 실패** (배포 서버의 `SESSION_SECRET` 이 로그인 시와 동일한지 확인).
  - `hasSignedSession: true`, `hasUser: false` → 세션 DB에 없거나 만료 (재로그인 또는 MongoDB 세션 확인).

### 5) 쿠키/세션 확인 (실제 도메인에서 로그인은 되는데 곧바로 로그아웃되는 경우)

- **로그인 응답에 Set-Cookie가 오는지**  
  개발자 도구 → **Network** → 로그인 요청(`POST .../api/login`) 선택 → **Response Headers** 에 `Set-Cookie: session=...` 이 있는지 확인.
- **그 다음 요청에 쿠키가 가는지**  
  같은 탭에서 `/api/me` 등 아무 API 요청 선택 → **Request Headers** 에 `Cookie: session=...` 이 있는지 확인.
- **접속 주소 통일**  
  `www.도메인.com` 과 `도메인.com` 을 섞어 쓰면 쿠키가 안 붙을 수 있으므로, 한 가지 주소만 사용해 보세요.
...
- **실제 도메인에서만 안 됨** → [deploy-render.md](./deploy-render.md) 의 **"6. 쿠키/세션 설정"** 섹션 참고.  
  - **토큰 전달**: 이 서비스는 **쿠키만** 사용합니다. Authorization 헤더/JWT/LocalStorage는 쓰지 않으므로, 401 원인은 쿠키 미전달 가능성이 큼.  
  - **프론트와 API 주소가 다름**(예: tornfi.com vs api.tornfi.com) → SameSite 때문에 쿠키 누락될 수 있음. 문서 6번의 **"3) 도메인 간 세션 불일치"** 참고 (`COOKIE_DOMAIN`, `COOKIE_SAMESITE=none`, `credentials: 'include'`).
