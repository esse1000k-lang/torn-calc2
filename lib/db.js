...
/** MongoDB 전용: id로 회원 문서 1건 삭제. writeUsers(filtered)만으로는 제거된 회원이 DB에서 삭제되지 않으므로 강퇴 시 호출. */
async function mongoDeleteUserById(userId) {
  if (!UserModel) return;
...
/** 회원 1명 삭제. Mongo: 문서 삭제. 파일 모드: writeUsers에서 이미 제외했으므로 no-op. */
async function deleteUserById(userId) {
  if (useMongo()) {
    if (!UserModel) await connectMongo();
    await mongoDeleteUserById(userId);
  }
  invalidateFileCache('users');
}

/** ID, displayName, walletAddress 중복 제거: 동일 _id/displayName/walletAddress를 가진 유저가 여러 개 있으면 마지막 항목만 남김 */
async function deduplicateUsers() {
  const users = await readUsers();
  if (!Array.isArray(users) || users.length === 0) return { removed: 0, count: users.length };
  
  // _id 기준 중복 제거 (MongoDB 호환)
  const byId = new Map();
  for (const u of users) {
    const id = u._id != null ? String(u._id) : (u.id != null ? String(u.id) : '');
    byId.set(id, u);
  }
  const dedupedById = Array.from(byId.values());
  
  // displayName 기준 중복 제거
  const byDisplayName = new Map();
  for (const u of dedupedById) {
    const displayName = u.displayName || '';
    if (displayName) {
      byDisplayName.set(displayName, u);
    } else {
      const userId = u._id != null ? String(u._id) : (u.id != null ? String(u.id) : '');
      byDisplayName.set(`no-name-${userId}`, u);
    }
  }
  const dedupedByDisplayName = Array.from(byDisplayName.values());
  
  // walletAddress 기준 중복 제거 (REGISTER_NO_WALLET 제외)
  const byWalletAddress = new Map();
  for (const u of dedupedByDisplayName) {
    const walletAddress = u.walletAddress || '';
    if (walletAddress && walletAddress !== 'REGISTER_NO_WALLET') {
      byWalletAddress.set(walletAddress, u);
    } else {
      const userId = u._id != null ? String(u._id) : (u.id != null ? String(u.id) : '');
      byWalletAddress.set(`no-wallet-${userId}`, u);
    }
  }
  const deduped = Array.from(byWalletAddress.values());
  
  const removed = users.length - deduped.length;
  if (removed > 0) {
    await writeUsers(deduped);
    console.log(`읽어온 유저 수: ${deduped.length}`);
  }
