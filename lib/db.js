...
async function migrateFileToMongo() {
  ...
  for (const user of users) {
    ...
    if (id == null) continue;
    await UserModel.updateOne({ id }, { $set: stripMongoMeta(user) }, { upsert: true });
  }
  ...
  for (const token of sessionTokens) {
    ...
    if (data && typeof data === 'object') await mongoSetSession(token, data);
  }
  if (sessionTokens.length) console.log('  - sessions:', sessionTokens.length);
