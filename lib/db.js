...
function setupFileWatchers() {
  ...
  try {
    ...
    console.log('Data file watchers active (changes on disk will be reflected on next read).');
  } catch (err) {
...
async function connect() {
  ...
  try {
    ...
    if ((mode & 0o002) !== 0) {
      console.warn('Security: data directory is world-writable. Restrict to server process only (e.g. chmod 700 data). See docs/security.md.');
    } else {
      console.log('Data directory: restrict permissions to server process only (e.g. chmod 700 data). See docs/security.md.');
    }
  } catch (_) {
    console.log('Data directory: restrict permissions to server process only (e.g. chmod 700 data). See docs/security.md.');
  }
