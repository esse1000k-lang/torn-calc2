...
async function fileGetFeedPosts(limit, offset) {
  const posts = await fileReadFeedPosts();
...
async function mongoGetFeedPosts(limit, offset) {
  const total = await FeedModel.countDocuments();
  const docs = await FeedModel.find().sort({ createdAt: -1 }).skip(offset).limit(limit).lean();
  const posts = [];
  for (const d of docs) {
...
async function getFeedPosts(limit, offset) {
  return useMongo() ? mongoGetFeedPosts(limit, offset) : fileGetFeedPosts(limit, offset);
}
